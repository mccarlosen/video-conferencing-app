...
<template>
	<form @submit.prevent="submitForm" class="space-y-3 sm:space-y-4 text-sm sm:text-base">
		...
		<div class="flex flex-col">
			...
			<p class="text-xs sm:text-sm font-light text-gray-600">Get an user access key from starting a meeting via the API or use one from an active meeting.</p>
			<p v-for="error of v$.accessKey.$errors" :key="error.$uid" class="text-sm font-light text-red-600" v-text="error.$message"></p>
		</div>
		<div class="flex justify-end">
			<button class="rounded bg-c1-500 text-black transition duration-150 ease-in-out hover:bg-c1-400 px-4 py-2" :disabled="sendingForm">Join to Meeting</button>
		</div>
	</form>
</template>