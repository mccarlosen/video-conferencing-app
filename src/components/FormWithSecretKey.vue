...
<template>
  <form @submit.prevent="submitForm()" class="space-y-5">
    ...
    <div class="flex flex-col">
      ...
      <input v-model="secretKey" type="text" id="secret-key" class="input-field mb-2" placeholder="Example: XZim5U3QlkPgaDjXln3fr332kKU9lBeRFSin0XcSio">
      <p v-for="error of v$.secretKey.$errors" :key="error.$uid" class="text-sm font-light text-red-600" v-text="error.$message"></p>
    </div>
    <div class="flex justify-end">
      <button class="rounded bg-c1-500 text-black transition duration-150 ease-in-out hover:bg-c1-400 px-4 py-2">Get Started</button>
    </div>
  </form>
</template>